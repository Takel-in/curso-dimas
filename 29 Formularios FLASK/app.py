...
def addstreamer():
    try:
        ...
        folowers = request.form["folowers"]

        streamer = Streamers(name, int(subs), int(folowers))    
        db.session.add(streamer)
        db.session.commit()
        return jsonify(streamer.serialize()), 200

    except Exception:
        exception("\n[SERVER]: Error un route /api/addstreamer. LOG:")
        return jsonify({"msg": "Algo ha salido mal"}), 500



@app.route("/api/searchstreamer", methods=["GET"])  #Indica donde se tiene que ejecutar la función que en este caso es en la raiz de la aplicación 
def searchstreamer():
    return render_template("searchstreamer.html")
...
def searchStreamerForm():
    #Pasamos el valor que queremos en la ruta con clave=valor en este caso name=alexelcapo   - Sa separación entre ruta y valores con un ?
    try:  
        nameStreamer = request.form["name"] #Esto nos ventra de la URL.
        ...
        print ("[SERVER]: error")
        exception("[SERVER]: error in route api //api/searchstreamer -> ")
        return jsonify ({"msg": "Ha ocurrido un error"}), 500
